...
app.post('/verify', (req, res) => {
  try {
    ...
    if (status === 'success' && isValid) {
      ...
      response.message = 'Payment pending or unknown status';
    }
...
app.get('/test', (req, res) => {
  ...
  res.json({
    ...
    test_data: {
      ...
      api_url: '/create',
      method: 'POST',
      required_fields: ['amount', 'firstName', 'email', 'phone']
    },
    curl_test: `curl -X POST https://YOUR_DOMAIN/create -H "Content-Type: application/json" -d '${JSON.stringify(testData)}'`
  });
});
